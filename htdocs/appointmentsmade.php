...
<nav class="navbar navbar-expand-lg navbar-light bg-white container-fluid" id="gtco-main-nav">
    <div class="container">
      ...
      <div id="my-nav" class="collapse navbar-collapse">
        ...
        <!-- Cart, Login, Sign Up -->
        <div class="cart-login-wrapper ml-lg-auto d-flex flex-column flex-lg-row align-items-start align-items-lg-center mt-3 mt-lg-0 pr-lg-3">
          ...
          <?php if (isset($_SESSION['user_id'])): ?>
            ...
            <a href="logout.php" class="btn btn-danger btn-login-signup text-uppercase">Logout</a>
          <?php else: ?>
  ...
  <h2 class="shawty">Your Appointments</h2>
  <?php if ($appointment->num_rows > 0): ?>
    <?php while ($row = $appointment->fetch_assoc()): ?>
      <form method="POST" action="appointment_alter.php" class="mb-4 p-3 border rounded shadow-sm" style="background-color: #f9f9f9;">
      <input type="hidden" name="appointment_id" value="<?= (int)$row['appointment_id'] ?>">
        ...
        <div class="form-group">
          <label><strong>Appointment ID Number:</strong></label>
          <p><?= htmlspecialchars($row['appointment_id']) ?></p>
        </div>



        <div class="form-group">
          <label for="service_date"><strong>Date & Time:</strong></label>
          <input type="datetime-local" name="service_date" class="form-control"
                 value="<?= date('Y-m-d\TH:i', strtotime($row['service_date'])) ?>" required>
        </div>
        ...
        <div class="form-group d-flex justify-content-end">
          <button type="submit" name="action" value="update" class="btn btn-sm btn-success mr-2">Update</button>
