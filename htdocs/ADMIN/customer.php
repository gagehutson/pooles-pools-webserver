...
<head>
  ...
  <title>Customers - Poole's Pools Admin Dashboard</title>
        ...
        <div class="bg-white tm-block h-100">
          <div class="row mb-3">
            <div class="col-md-8 col-sm-12">
              <h2 class="tm-block-title d-inline-block">Customer Directory</h2>
            </div>
            <div class="col-md-4 col-sm-12 d-flex justify-content-end align-items-center">
              <input style="border-radius: 20px;" type="text" class="form-control" placeholder="Search customers..." id="customerSearchInput">
            </div>
          </div>

          <div class="table-responsive">
                        <table class="table table-hover table-striped tm-table-striped-even mt-3" id="customersTable">
                            <thead>
                            <tr class="tm-bg-gray">
                                <th scope="col" class="text-center" style="border-top-left-radius: 20px;">Customer ID Number</th>
                                <th scope="col" class="text-center">Customer Name</th>
                                <th scope="col" class="text-center">Email Address</th>
                                <th scope="col" class="text-center">Phone</th>
                                <th scope="col" class="text-center">Username</th>
                                <th scope="col" class="text-center">Pool IDs Associated</th>
                            ...
                            <?php
                              $query = $conn->query("
                                SELECT c.customer_id, c.firstname, c.lastname, c.email, c.phone, c.username,
                                      GROUP_CONCAT(DISTINCT p.pool_id SEPARATOR ', ') AS pool_ids,
                                      GROUP_CONCAT(DISTINCT a.appointment_id SEPARATOR ', ') AS appointment_ids
                                FROM customer c
                                LEFT JOIN pool p ON c.customer_id = p.customer_id
                            ...
                            ?>
                                ...
                                <td class="text-center border-right"><?= htmlspecialchars($row['firstname'] . ' ' . $row['lastname']) ?></td>
                                <td class="text-center border-right"><?= htmlspecialchars($row['email']) ?></td>
                                <td class="text-center border-right"><?= htmlspecialchars($row['phone']) ?></td>
                                <td class="text-center border-right"><?= htmlspecialchars($row['username']) ?></td>
                                <td class="text-center border-right"><?= htmlspecialchars($row['pool_ids'] ?? 'None') ?></td>
